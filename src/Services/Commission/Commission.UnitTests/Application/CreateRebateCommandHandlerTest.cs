...
namespace Commission.UnitTests.Application
{
    public class CreateRebateCommandHandlerTest : IClassFixture<AutoMapperFixture>
    {
        ...
        private readonly Mock<ICommissionIntegrationEventService> _integrationEventMock;
        ...
        public CreateRebateCommandHandlerTest(AutoMapperFixture fixture)
        {
            ...
            _integrationEventMock = new Mock<ICommissionIntegrationEventService>();
        ...
        private void CreateMockDbSet<T>(IQueryable<T> fakeData, Action<Mock<CommissionContext>> ctxCb) where T : Entity, IAggregateRoot
        {
        ...
        public async Task Command_handler_add_should_work_test()
        {
            ...
            var result = await handler.Handle(new CreateRebateCommand {List = fakeRequest.Rebates}, CancellationToken.None);
